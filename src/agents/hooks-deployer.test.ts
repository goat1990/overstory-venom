...
describe("deployHooks", () => {
	...
	test("includes sandbox settings when sandbox enabled", async () => {
		const worktreePath = join(tempDir, "sandbox-wt");
		await deployHooks(worktreePath, "sandbox-agent", "builder", {
			enabled: true,
		...
		expect(parsed.sandbox.network.allowedDomains).toEqual(["api.anthropic.com", "github.com"]);
	});

	test("maps denyReadPaths to permissions.deny Read rules", async () => {
		const worktreePath = join(tempDir, "deny-wt");
		await deployHooks(worktreePath, "deny-agent", "builder", {
			enabled: true,
			allowedDomains: [],
			denyReadPaths: ["~/.ssh", "~/.aws"],
		});
		...
		expect(parsed.permissions.deny).toEqual(["Read(~/.ssh/**)", "Read(~/.aws/**)"]);
	});

	test("omits sandbox settings when sandbox disabled", async () => {
		const worktreePath = join(tempDir, "no-sandbox-wt");
		await deployHooks(worktreePath, "no-sandbox-agent", "builder", {
			enabled: false,
			allowedDomains: [],
			denyReadPaths: [],
		});
	...
	test("omits sandbox settings when sandboxConfig not provided", async () => {
		const worktreePath = join(tempDir, "no-config-wt");
		await deployHooks(worktreePath, "no-config-agent", "builder");
		const outputPath = join(worktreePath, ".claude", "settings.local.json");
		const content = await Bun.file(outputPath).text();
		const parsed = JSON.parse(content);
		expect(parsed.sandbox).toBeUndefined();
	...
	test("omits permissions.deny when denyReadPaths is empty", async () => {
		const worktreePath = join(tempDir, "no-deny-wt");
		await deployHooks(worktreePath, "no-deny-agent", "builder", {
			enabled: true,
